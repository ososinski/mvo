...
the_chosen.push_back(point3d(11,2,0));
          ...
          if (octree.isNodeAtThreshold(*it)) {
              ...
              for(int l = 0;l<6;l++){                 
                  ...
                  if (face_visibility[l]>0){
                     ...
                     if (face_visibility[l]>0.5){
                        full_vis++;
                        fvq += (face_visibility[l]-0.5)*2;
                     }else{
                        part_vis++;  
                     }
 ...
 std::cout <<"% local_visibility("<<(*itr).x()<<","<<(*itr).y()<<","<<(*itr).z()<<")="<<global_visibility/global_points<<" fully visible("<<full_vis/global_points<<" of quality "<<fvq/full_vis<<") partially visible ("<<part_vis/global_points<<")"<<endl;  
